...
class Picture_uploader extends Trongate {
    ...
    function ditch($update_id) {
        ...
        $picture_name = $this->input('picture_name');
        $calling_module = $this->input('calling_module');
        $targetColumnName = $this->input('targetColumnName');
        $destination = $this->input('destination');
        $thumbnailDir = $this->input('thumbnailDir');
    ...
    function submit_upload_picture($update_id) {
        ...
        if ($submit == 'Upload') {
            ...
            $max_file_size = $this->input('maxFileSize', true);
            $max_width = $this->input('maxWidth', true);
            $max_height = $this->input('maxHeight', true);
            ...
            if ($result == true) {
                ...
                $config['destination'] = $this->input('destination', true);

                $config['max_width'] = $this->input('resizedMaxWidth', true);
                $config['max_height'] = $this->input('resizedMaxHeight', true);

                $thumbnail_dir = trim($this->input('thumbnailDir', true));
                $thumbnail_dir = str_replace(' ', '', $thumbnail_dir);
                if ($thumbnail_dir !== '') {
                    $config['thumbnail_dir'] = $this->input('thumbnailDir', true);
                    $config['thumbnail_max_width'] = $this->input('thumbnailMaxWidth', true);
                    $config['thumbnail_max_height'] = $this->input('thumbnailMaxHeight', true);
                }
                ...
                //update the database
                $table = $this->input('targetModule', true);
                $column = $this->input('targetColumnName', true);
                $data[] = $_FILES['picture']['name'];
                $data[] = $this->input('update_id', true);
