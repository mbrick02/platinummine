<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>store_accounts/create"><button class="w3-button w3-medium primary">
        ...
        <table class="w3-table results-tbl" id="results-tbl" style="margin-left: -2000em;">
            <thead>
                <tr class="primary">                    <th colspan="11">
                        <div class="table-top">
                            <div>   
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {        
      ...
      'OR company LIKE': '%' + searchPhrase + '%',        
      'OR street_address LIKE': '%' + searchPhrase + '%',        
      'OR address_line_2 LIKE': '%' + searchPhrase + '%',        
      'OR city LIKE': '%' + searchPhrase + '%',        
      'OR zip_code LIKE': '%' + searchPhrase + '%',        
      'OR telephone_number LIKE': '%' + searchPhrase + '%',        
      'OR email LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/store_accounts';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no store account records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/store_accounts';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>store_accounts/show/' + records[i]['id'];
      ...
      var newRow = `<tr>
                        ...
                        <td>${records[i]['street_address']}</td>
                        <td>${records[i]['address_line_2']}</td>
                        <td>${records[i]['city']}</td>
                        <td>${records[i]['state']}</td>
                        <td>${records[i]['zip_code']}</td>
