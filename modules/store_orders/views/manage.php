...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {        
      'tracking_url LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/store_orders';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no store order records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/store_orders';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>store_orders/show/' + records[i]['id'];
      var editBtn = '<a href="' + recordUrl + '"><button type="button" class="btn btn-xs">View</button></a>';
      var newRow = `<tr>
                        <td>${records[i]['tracking_url']}</td>
