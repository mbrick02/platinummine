<div class="w3-row">
    <div class="w3-container">
        ...
        <?= flashdata() ?>
        <p>
            <a href="<?= BASE_URL ?>store_order_status_levels/create"><button class="w3-button w3-medium primary">
...
function refreshResults(searchPhrase) {
  //update totalRows
    var params = {        
      'order_status_title LIKE': '%' + searchPhrase + '%'
    }

  var target_url = '<?= BASE_URL ?>api/count/store_order_status_levels';
...
function noResults() {
  ...
  if (searchPhrase !== '') {
    ...
    var msg = 'There are currently no store order status level records';
  }
...
function getRecords() {
  ...
  var target_url = '<?= BASE_URL ?>api/get/store_order_status_levels';
  ...
  http.onload = function () {
    ...
    for (var i = 0; i < records.length; i++) {
      records[i];
      var recordUrl = '<?= BASE_URL ?>store_order_status_levels/show/' + records[i]['id'];
      var editBtn = '<a href="' + recordUrl + '"><button type="button" class="btn btn-xs">View</button></a>';
      var newRow = `<tr>
                        <td>${records[i]['order_status_title']}</td>
                        <td>${editBtn}</td>
                    </tr>`;
